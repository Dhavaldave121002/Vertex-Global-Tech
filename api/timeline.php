...
switch($method) {
    case 'GET':
        try {
            // Check/Create Table
            $conn->exec("CREATE TABLE IF NOT EXISTS timeline (
            ...
            $stmt = $conn->prepare("SELECT * FROM timeline ORDER BY year ASC");
            $stmt->execute();
    ...
    case 'POST':
        ...
        try {
            ...
            $stmt = $conn->prepare("INSERT INTO timeline (year, title, description) VALUES (?, ?, ?)");
    ...
    case 'PUT':
        ...
        try {
            ...
            $stmt = $conn->prepare("UPDATE timeline SET year = ?, title = ?, description = ? WHERE id = ?");
