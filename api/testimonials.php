...
switch($method) {
    case 'GET':
        ...
        try {
            ...
            $check = $conn->query("SHOW TABLES LIKE 'testimonials'");
            if($check->rowCount() == 0) {
    ...
    case 'POST':
        ...
        try {
            $stmt = $conn->prepare("INSERT INTO testimonials (name, role, message, avatar) VALUES (?, ?, ?, ?)");
    ...
    case 'PUT':
        ...
        try {
            $stmt = $conn->prepare("UPDATE testimonials SET name = ?, role = ?, message = ?, avatar = ? WHERE id = ?");
            $success = $stmt->execute([
                $data['name'],
                $data['role'],
                $data['message'],
