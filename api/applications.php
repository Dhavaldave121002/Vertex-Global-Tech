...
switch($method) {
    case 'GET':
        try {
            ...
            $stmt = $conn->prepare("SELECT * FROM job_applications ORDER BY id DESC");
            $stmt->execute();
    ...
    case 'POST':
        ...
        try {
            ...
            $stmt = $conn->prepare("INSERT INTO job_applications (jobId, jobTitle, jobType, name, email, phone, message, resumeData, resumeFileName, status, submittedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
