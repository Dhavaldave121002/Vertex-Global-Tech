...
switch($method) {
    case 'GET':
        ...
        try {
            ...
            $check = $pdo->query("SHOW TABLES LIKE 'teams'");
            if($check->rowCount() == 0) {
    ...
    case 'POST':
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO teams (name, role, bio, image, social_links) VALUES (?, ?, ?, ?, ?)");
    ...
    case 'PUT':
        ...
        try {
            $stmt = $pdo->prepare("UPDATE teams SET name = ?, role = ?, bio = ?, image = ?, social_links = ? WHERE id = ?");
            $success = $stmt->execute([
                $data['name'],
                $data['role'],
                $data['bio'],
