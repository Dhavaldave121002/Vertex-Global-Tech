...
switch($method) {
    case 'GET':
        ...
        try {
            ...
            $check = $pdo->query("SHOW TABLES LIKE 'accounting'");
            if($check->rowCount() == 0) {
            ...
            $stmt = $pdo->prepare("SELECT * FROM accounting ORDER BY date DESC");
            $stmt->execute();
    ...
    case 'POST':
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO accounting (date, description, type, amount, category, fy) VALUES (?, ?, ?, ?, ?, ?)");
    ...
    case 'PUT':
        ...
        try {
            $stmt = $pdo->prepare("UPDATE accounting SET date = ?, description = ?, type = ?, amount = ?, category = ?, fy = ? WHERE id = ?");
            $success = $stmt->execute([
                $data['date'],
                $data['desc'],
                $data['type'],
    ...
    case 'DELETE':
        ...
        try {
            $stmt = $pdo->prepare("DELETE FROM accounting WHERE id = ?");
