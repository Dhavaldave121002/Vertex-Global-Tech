...
export default function Blog() {
  ...
  return (
    <div className="bg-[#030712] min-h-screen relative overflow-x-hidden">
      {/* Background Ambience */}
            ...
            >
              {filteredPosts.map((post, index) => (
                <motion.article
                  ...
                  className="group bg-[#0f172a] rounded-2xl overflow-hidden border border-white/5 hover:border-blue-500/30 transition-all duration-300 hover:shadow-2xl hover:shadow-blue-900/10 cursor-pointer flex flex-col h-full"
                >
                  {/* Image Container */}
                  <div className="h-64 overflow-hidden relative">
                    <div className="absolute inset-0 bg-blue-900/10 mix-blend-overlay z-10 group-hover:bg-transparent transition-all duration-500"></div>
                    <img
                      src={post.image}
                      alt={post.title}
                    ...
                    <div className="absolute top-4 left-4 z-20">
                      <span className="bg-black/60 backdrop-blur-md border border-white/10 text-white text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-2">
                  ...
                  <div className="p-8 flex flex-col flex-grow relative">
                    <div className="text-xs font-semibold text-gray-500 mb-4 flex items-center gap-2">
                      <FaCalendarAlt className="text-gray-600" /> {post.date}
                    </div>

                    <h3 className="text-2xl font-bold text-white mb-4 leading-tight group-hover:text-blue-400 transition-colors line-clamp-2">
                      {post.title}
                    </h3>
                    ...
                    <div className="mt-auto pt-6 border-t border-white/5 flex items-center text-blue-500 font-bold text-sm tracking-wide group-hover:translate-x-2 transition-transform">
      ...
      <AnimatePresence>
        {selectedPost && (
          <>
            ...
            >
              <div
                className="bg-[#0f172a] border border-white/10 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl pointer-events-auto relative flex flex-col"
                onClick={(e) => e.stopPropagation()}
              >
                {/* Close Button */}
                <button
                  onClick={() => setSelectedPost(null)}
                  className="absolute top-6 right-6 z-50 p-2.5 bg-black/50 hover:bg-black/80 rounded-full text-white transition-colors border border-white/10"
                >
                  <FaTimes />
                </button>
                ...
                <div className="px-8 md:px-16 pb-16 -mt-20 relative">
                  <div className="flex items-center gap-4 text-sm font-bold text-blue-400 mb-6">
                    <span className="bg-blue-500/10 px-4 py-1.5 rounded-full border border-blue-500/20">
                  ...
                  <div className="prose prose-lg prose-invert max-w-none text-gray-300 leading-loose">
                    <p className="font-medium text-xl text-gray-200 mb-10">{selectedPost.excerpt}</p>
                    <p>{selectedPost.content}</p>
                    <h3 className="text-2xl font-bold text-white mt-8 mb-4">Key Takeaways</h3>
                    <ul className="list-disc pl-5 space-y-2 mb-8">
                      <li>Understanding the core principles of {selectedPost.category}.</li>
                      <li>Implementing best practices for scalability.</li>
                      <li>Leveraging new tools to improve workflow efficiency.</li>
                    </ul>
                    <p>
