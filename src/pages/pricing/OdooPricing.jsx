...
export default function OdooPricing() {
  ...
  useEffect(() => {
    const loadData = () => {
      const localPlans = localStorage.getItem('vgtw_odoo_pricing_plans');
      if (localPlans) setPlans(JSON.parse(localPlans));

      const localComp = localStorage.getItem('vgtw_odoo_pricing_comparison');
      if (localComp) setTableFeatures(JSON.parse(localComp));

      const localFaqs = localStorage.getItem('vgtw_odoo_pricing_faq');
